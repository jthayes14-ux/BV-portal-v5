  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fff', fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      ...
      <main className="dashboard-main" style={{ maxWidth: 600, margin: '0 auto', padding: '48px 24px' }}>
        ...
        <h1 style={{ fontSize: 32, fontWeight: 800, color: '#111827', marginBottom: 8, letterSpacing: '-0.02em' }}>Settings</h1>
        <p style={{ fontSize: 15, color: '#6B7280', marginBottom: 32 }}>Manage your address and payment information</p>

        {/* Profile info */}
        <div style={{
          background: '#F9FAFB', borderRadius: 20, padding: '28px',
          marginBottom: 28,
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{
              width: 52, height: 52, background: '#6366F1', borderRadius: '50%',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            ...
            <div>
              {userName && <p style={{ fontSize: 17, fontWeight: 700, color: '#111827', letterSpacing: '-0.01em' }}>{userName}</p>}
              <p style={{ fontSize: 14, color: '#6B7280', marginTop: 2 }}>{userEmail}</p>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 28, background: '#F3F4F6', borderRadius: 10, padding: 4 }}>
          <button onClick={() => { setActiveSection('address'); setMessage(''); setError(''); }} style={{
            ...
            background: activeSection === 'address' ? '#6366F1' : 'transparent',
            color: activeSection === 'address' ? '#fff' : '#6B7280',
          ...
          <button onClick={() => { setActiveSection('payment'); setMessage(''); setError(''); }} style={{
            flex: 1, padding: '12px 20px', fontSize: 14, fontWeight: 600,
            background: activeSection === 'payment' ? '#6366F1' : 'transparent',
            color: activeSection === 'payment' ? '#fff' : '#6B7280',
            border: 'none', borderRadius: 8, cursor: 'pointer',
            transition: 'all 0.2s ease',
          }}>
            Payment
        ...
        {message && (
          <div style={{
            ...
            marginBottom: 20, fontSize: 14, fontWeight: 500, color: '#059669',
          }}>
            {message}
          </div>
        )}
        ...
        {activeSection === 'address' && (
          <div style={{ background: '#F9FAFB', borderRadius: 20, padding: '32px 28px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h2 style={{ fontSize: 20, fontWeight: 700, color: '#111827', letterSpacing: '-0.01em' }}>Address</h2>
              {!editingAddress && (
                <button
                  onClick={() => { setEditingAddress(true); setMessage(''); setError(''); }}
                  style={{
                    padding: '10px 24px', fontSize: 14, fontWeight: 600,
                    background: '#F3F4F6', border: '1px solid #E5E7EB', borderRadius: 100,
            ...
            {!editingAddress ? (
              ...
              address || city || state || zip ? (
                <div style={{
                  ...
                  <div style={{
                    width: 40, height: 40, borderRadius: 12, background: '#E5E7EB',
                  ...
                  <div>
                    {address && <p style={{ fontSize: 15, fontWeight: 600, color: '#111827', marginBottom: 4 }}>{address}</p>}
                    {(city || state || zip) && (
                ...
                <div style={{ textAlign: 'center', padding: '48px 24px' }}>
                  <div style={{
                    width: 64, height: 64, borderRadius: 20, background: '#E5E7EB',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    margin: '0 auto 20px',
                  }}>
                  ...
                  <p style={{ fontSize: 16, fontWeight: 600, color: '#111827', marginBottom: 6 }}>No address on file</p>
                  <p style={{ fontSize: 14, color: '#6B7280', marginBottom: 24, lineHeight: 1.5 }}>Your address will be added automatically when you complete your first booking</p>
                  <button
                    onClick={() => setEditingAddress(true)}
                    style={{
                      padding: '14px 32px', fontSize: 15, fontWeight: 600,
                      background: '#6366F1', border: 'none', borderRadius: 100,
              ...
              <form onSubmit={handleSaveAddress}>
                <div style={{ marginBottom: 24 }}>
                  <label style={{
                    display: 'block', fontSize: 13, fontWeight: 700, color: '#111827',
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <button
                    ...
                    style={{
                      padding: '14px 32px', fontSize: 15, fontWeight: 600,
                      background: saving ? '#e5e5e5' : '#6366F1',
        ...
        {activeSection === 'payment' && (
          <div style={{ background: '#F9FAFB', borderRadius: 20, padding: '32px 28px' }}>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: '#111827', letterSpacing: '-0.01em', marginBottom: 24 }}>Payment Method</h2>
            {cardLast4 ? (
              <div style={{
                ...
                <div>
                  <p style={{ fontSize: 15, fontWeight: 700, color: '#111827' }}>{cardBrand || 'Card'} ending in {cardLast4}</p>
                  <p style={{ fontSize: 13, color: '#6B7280', marginTop: 4 }}>Update your payment method during your next booking</p>
                </div>
              </div>
            ) : (
              <div style={{ textAlign: 'center', padding: '48px 24px' }}>
                ...
                <p style={{ fontSize: 16, fontWeight: 600, color: '#111827', marginBottom: 6 }}>No payment method on file</p>
                <p style={{ fontSize: 14, color: '#6B7280', lineHeight: 1.5 }}>Your payment information will be saved when you complete your first booking</p>
              </div>
            )}
          </div>
        )}
